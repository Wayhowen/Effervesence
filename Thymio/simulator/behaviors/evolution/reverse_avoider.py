...
class ReverseAvoider(Behavior):
    def __init__(self, simulator, controller, q_table, total_steps):
        super().__init__(simulator, controller, q_table)
        self.states = (
            "ALLBACK", "BACK", "LEFT", "RIGHT", "LEFTBACK",
            "RIGHTBACK", "LEFTRIGHT", "NOTHING", "BEHIND", "SAFE", "CLOSE_LEFT", "CLOSE_MIDDLE", "CLOSE_RIGHT"
        )
        self.actions = (
            "GOFORWARDS", "GOLEFT", "GORIGHT", "SLOW_BACKWARDS_LEFT", "SLOW_BACKWARDS_RIGHT", "LEAN_LEFT",
            "LEAN_RIGHT", "SLOW_FORWARDS_LEFT", "SLOW_FORWARDS_RIGHT", "SLOW_FORWARDS", "STOP", "BACKWARDS",
    ...
    def get_next_state(self, closest_reading, other_robot_camera_positions: Dict[str, Behavior]):
        if self.is_in_safezone:
        ...
        elif all(other_robot_camera_positions[k] and other_robot_camera_positions[k].color in ["seeking", "safe_seeking"] for k in ["l","m","r"]):
            return self.states.index("ALLBACK")
        elif all(other_robot_camera_positions[k] and other_robot_camera_positions[k].color in ["seeking", "safe_seeking"] for k in ["l","m"]):
            return self.states.index("LEFTBACK")
        elif all(other_robot_camera_positions[k] and other_robot_camera_positions[k].color in ["seeking", "safe_seeking"] for k in ["m","r"]):
            return self.states.index("RIGHTBACK")
        elif all(other_robot_camera_positions[k] and other_robot_camera_positions[k].color in ["seeking", "safe_seeking"] for k in ["l","r"]):
            ...
            return self.states.index("CLOSE_LEFT")
        elif closest_reading[2] < 0.09:
            return self.states.index("CLOSE_MIDDLE")
        elif closest_reading[3] < 0.09 or closest_reading[4] < 0.09:
            return self.states.index("CLOSE_RIGHT")
