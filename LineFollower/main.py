    ...
    def _prepare_commands_to_reach_position(self,
        ...
        if self._direction != direction_to_go:
            ...
            if (direction_to_go == right_directions[(current_position_index + 1) % 4]):
                ...
                self._direction = right_directions[(right_directions.index(self._direction) + 1) % 4]
...
if __name__ == "__main__":
    ...
    for command in commands[1:]:
        direction_to_go = controls._choose_direction_to_go(command)

        commands_to_perform = controls._prepare_commands_to_reach_position(
            command, direction_to_go)
        robot_commands.extend(commands_to_perform)
    
    print(robot_commands)
    shortened_commands = []
    skip_next = False
    for i, rc in enumerate(robot_commands):
        if i <= len(robot_commands):
            if skip_next:
                skip_next = False
            elif robot_commands[i + 1] == rc and rc == "push":
                shortened_commands.append("double_push")
                skip_next = True
            else:
    ...
    for robot_command in shortened_commands:
        controls.perform_command(robot_command)
